...
cv::Point2d tl(  -3.6883697,  40.9166661 );
cv::Point2d tr(   -3.5208329, 40.9175445);
cv::Point2d bl(  -3.6874979, 40.8324559 );
cv::Point2d br( -3.5201733,  40.8333317 );

// determine dem corners
cv::Point2d dem_bl( -3.6881451,40.8319589);
cv::Point2d dem_tr(-3.5202371,40.9180035);
...
if(x==(int(iWidth/2)) && y==(int(iHeight/2)) )
{
origen_z= demcropped.at<short>({(x),(y)});
circle(realimage, Point(x, y), 3, Scalar(0,0,0), FILLED,LINE_8,0); 
circle(miniLaCabrera, Point(x, y), 3, Scalar(0,0,0), FILLED,LINE_8,0); 
circle(goalsImage, Point(x, y), 3, Scalar(0,0,0), FILLED,LINE_8,0); 
circle(realimage, Point(x, y), 3, Scalar(0,0,0), FILLED,LINE_8,0); 
circle(miniDem2, Point(x, y), 3, Scalar(0,0,0), FILLED,LINE_8,0); 
circle(miniDem, Point(x, y), 3, Scalar(0,0,0), FILLED,LINE_8,0); 
}
...
fp2 = fopen("/home/juanmat/catkin_ws/src/path_planning_sims/path_planning_intro/unit4_pp/maps/map_coords.txt","w");
...
{
FILE *fpx;
fpx = fopen( "/home/juanmat/catkin_ws/src/path_planning_sims/path_planning_intro/unit4_pp/maps/map_path.txt","r");
realimage_clean = cv::imread("/home/juanmat/Escritorio/final/xxxm/mapa_XXXm_real.tif",cv::IMREAD_LOAD_GDAL| cv::IMREAD_COLOR );
resize(realimage_clean,realimage_clean, dem.size(),INTER_LINEAR);
fscanf(fpx, "%*[^\n]");
int aux;
float x1,y1,x2,y2;
...
fclose(fpx);
//cv::imwrite("/home/juanmat/Escritorio/mapa_sat_planned.jpg",realimage_clean);
//imshow("Final_Path",realimage_clean);

cv::imwrite("/home/juanmat/Escritorio/final/mapa_sat_planned_1.jpg",goalsImage);
cv::imwrite("/home/juanmat/Escritorio/final/mapa_sat_planned_2.jpg",miniLaCabrera);
cv::imwrite("/home/juanmat/Escritorio/final/mapa_sat_planned_3.jpg",realimage);
cv::imwrite("/home/juanmat/Escritorio/final/mapa_sat_planned_4.jpg",miniDem2);
cv::imwrite("/home/juanmat/Escritorio/final/mapa_sat_planned_5.jpg",miniDem);
}
...
image = cv::imread("/home/juanmat/Escritorio/final/xxxm/mapa_XXXm.tif",cv::IMREAD_LOAD_GDAL| cv::IMREAD_COLOR );
realimage = cv::imread("/home/juanmat/Escritorio/final/xxxm/mapa_XXXm_real.tif",cv::IMREAD_LOAD_GDAL| cv::IMREAD_COLOR );
//Mat image_show = cv::imread("/home/robcib/Desktop/lacabrera/lacabrera.jpg",cv::IMREAD_LOAD_GDAL| cv::IMREAD_COLOR );
dem = cv::imread("/home/juanmat/Escritorio/final/xxxm/prueba_XXXm.tif", cv::IMREAD_LOAD_GDAL| cv::IMREAD_ANYDEPTH );
...
fp3= fopen("/home/juanmat/catkin_ws/src/cv_gdal/src/gdalinfo.txt","r");
fscanf(fp3,"%lf %lf tl\n",&originX,&originY);	
fscanf(fp3,"%lf %lf br\n",&X_geo_br,&Y_geo_br);
fscanf(fp3,"%lf %lf bl\n",&X_geo_bl,&Y_geo_bl);
...
key = (char)waitKey(0);
   ...
   if(key==49)
	 ...
	 {
	 //destroyAllWindows();
	 cv::imwrite("/home/juanmat/catkin_ws/src/path_planning_sims/path_planning_intro/unit4_pp/maps/unitX_map.pgm",miniDem);
	 cv::imwrite("/home/juanmat/catkin_ws/src/navigation_data/unitX_map.pgm",miniDem);
 	 cv::imwrite("/home/juanmat/catkin_summit_xl/src/summit_xl_common/summit_xl_localization/maps/real/unitX_map.pgm",miniDem);
	 printf("map_saved");
	 printf("\n");
